...
if(isset($_POST['login']))
{
    ...
    if($num>0)
    {
        $final_query= mysqli_query($con,"SELECT id,fname FROM users WHERE email='$useremail' and password='$dec_password' and is_approve=1");
        $final_query_num=mysqli_fetch_array($final_query);
        if($final_query_num>0){
            $_SESSION['id']=$num['id'];
            $_SESSION['name']=$num['fname'];
            header("location:welcome.php");
        }else{
            echo "<script>alert('Your account is not approved. Please contact with administrator');</script>";
        }
    }
    else
    {
